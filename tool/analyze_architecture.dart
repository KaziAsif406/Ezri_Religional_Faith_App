...
void checkApiLayer(Directory libDir, String root) {
  ...
  for (final File file in apiFiles) {
    ...
    if (clean.contains('_singleton') &&
      ...
      fail(
        cat,
        '$rel does NOT use the required singleton pattern',
        'Must have _singleton, _internal(), and get instance',
      );
    }

    // Uses pre-configured Dio wrappers
    final RegExp dioWrappers = RegExp(
      r'\b(getHttp|postHttp|putHttp|deleteHttp|patchHttp)\b',
    );
    if (dioWrappers.hasMatch(clean)) {
      ...
      fail(
        cat,
        '$rel does NOT use getHttp/postHttp/putHttp/patchHttp/deleteHttp',
    ...
    if (clean.contains('endpoints.dart')) {
      ...
      fail(
        cat,
        '$rel does NOT import endpoints.dart',
        'Endpoint URLs must come from Endpoints class',
      );
    }

    // No hardcoded URLs
    final RegExp hardcodedUrl = RegExp(
      r'''['"]https?://''',
      caseSensitive: false,
    );
    if (hardcodedUrl.hasMatch(clean)) {
      fail(
        cat,
        '$rel contains hardcoded URL strings',
...
void checkRxLayer(Directory libDir, String root) {
  ...
  for (final File file in rxFiles) {
    ...
    if (clean.contains('extends RxResponseInt')) {
      ...
      fail(
        cat,
        '$rel does NOT extend RxResponseInt',
...
void checkEndpointCentralization(Directory libDir, String root) {
  ...
  final RegExp apiPathLiteral = RegExp(
    r'''['"]/(?:api|auth|user|products?|v\d)[/'"?]''',
  );
...
void checkCodeQuality(Directory libDir, String root) {
  ...
  final List<File> featureFiles = _dartFiles(
    Directory('${libDir.path}/features'),
  ).cast<File>();
  ...
  final RegExp edgeInsetsRaw = RegExp(r'EdgeInsets\.\w+\(\s*(\d+)(?![\d.])');
  ...
  final RegExp rawColors = RegExp(
    r'\bColors\.(?!transparent\b|black\b|white\b)\w+',
  );
  ...
  // ── 5c. Navigation ──
  final RegExp navViolation = RegExp(
    r'Navigator\s*\.\s*(?:of|push|pop|replace)',
  );
  int navViolations = 0;
  ...
  final RegExp forbiddenState = RegExp(
    r"import\s+['"
    r""""]package:(?:provider|riverpod|flutter_bloc)/""",
  );
  ...
  if (printCount == 0) {
    ...
    warn(
      cat,
      '$printCount print() call(s) found — prefer dart:developer log()',
...
void checkDiAndRoutes(Directory libDir, String root) {
  ...
  if (invalidImports == 0) {
    pass(
      cat,
      'all_routes.dart — all $validImports import(s) resolve correctly',
    );
  }
...
void printReport() {
  ...
  for (final MapEntry<String, List<CheckResult>> entry in grouped.entries) {
    ...
    final int warnings = checks
        .where((c) => c.severity == Severity.warning)
        .length;
    final int failures = checks
        .where((c) => c.severity == Severity.fail)
        .length;
  ...
  // Summary
  final int totalPassed = results
      .where((r) => r.severity == Severity.pass)
      .length;
  final int totalWarnings = results
      .where((r) => r.severity == Severity.warning)
      .length;
  final int totalFails = results
      .where((r) => r.severity == Severity.fail)
      .length;
